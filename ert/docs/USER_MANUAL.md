...
**构建脚本功能**：
- `build.bat` - 构建所有版本（GUI + CLI）
- `build.bat gui` - 仅构建 GUI 版本
- `build.bat cli` - 仅构建 CLI 版本
- `build.bat clean` - 清理构建产物
- `build.bat check` - 代码检查

构建产物输出到 `bin/` 目录：
- `bin\ert.exe` - GUI 版本
- `bin\ert-cli.exe` - CLI 版本
...
# 编译 GUI 版本
go build -ldflags="-s -w -H=windowsgui -trimpath" -o bin\ert.exe ./cmd/gui/

# 编译 CLI 版本
go build -ldflags="-s -w -trimpath" -o bin\ert-cli.exe ./cmd/cli/
```

#### 4. 可选：使用 Wails 构建（需要 Node.js）
